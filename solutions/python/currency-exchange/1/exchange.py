...
def exchangeable_value(budget, exchange_rate, spread, denomination):
    ...
    # 1. Convert spread from integer percentage to decimal
    spread_decimal = spread / 100
    
    # 2. Calculate the actual exchange rate including the spread fee
    actual_exchange_rate = exchange_rate * (1 + spread_decimal)
    
    # 3. Calculate the raw maximum converted value
    total_value = budget / actual_exchange_rate
    
    # 4. Round down to the nearest full denomination (bills cannot be divided)
    max_denominations = int(total_value // denomination)
    
    # 5. Return the final total value as an integer
    return max_denominations * denomination
